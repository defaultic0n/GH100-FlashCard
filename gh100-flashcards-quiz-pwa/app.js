...
function savePrefs(){
  localStorage.setItem(PREF_KEY, JSON.stringify({onlyMissed: state.onlyMissed, shuffle: state.shuffle, quizMode: state.quizMode}));
}
...
function correctIndices(card){
  const idxs = [];
  (card.options||[]).forEach((o,i)=>{ if(o.isCorrect) idxs.push(i); });
  return idxs;
}

function requiredCount(card){
  const ci = correctIndices(card);
...
function formatFrontQuiz(card){
  ...
  const btns = (card.options||[]).map((o,i)=>{
...
function updateButtonsForMode(){
  ...
  if(state.quizMode){
    ...
    flip.disabled = !state.checked;   // prevents spoilers before answering
...
function submitQuiz(){
  ...
  const correct = new Set(correctIndices(card));
